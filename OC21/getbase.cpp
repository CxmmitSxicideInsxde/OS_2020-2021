...
{
    cout << to_string(ppt->key) + " " + ppt->LastName + " " + ppt->FirstName + " " + ppt->LastName + 
    " " + ppt->subject + " " + to_string(ppt->Ocenka) << endl;
}
    ...
    while(idMenu != 0)
    {
        switch(idMenu)
        {
            ...
            case 3:
            {
                ...
                inputOcenka = getIntR(2,5);
                for(int i = 0; i < validEntry; i ++)
                {
                    tmpptr = ptr + i;  
                    if(tmpptr->Ocenka == inputOcenka)output(tmpptr); // пробегаемся и сравниваем оценки
                }
            }
        }
        idMenu = getIdMenu();
