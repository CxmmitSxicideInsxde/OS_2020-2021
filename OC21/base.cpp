...
int getInt();
int getIntR(int form, int before);
...
struct base // заводим структуру
{
    ...
    char MiddleName[32];
    char subject[32];
} *ptr, *tmpptr; /* Открываем файл или сначала создаем его (если такого файла не было). Права доступа к файлу
...
int main() {
    ...
    size_t lengths;  /* Длина отображаемой части файла */
    ...
    lengths = 300*sizeof(struct base); // Можно сделать 300 записей
    ...
    while(idMenu != 0)
    {
        switch(idMenu)
        {
            ...
            case 2:
            {
                ...
                inputKey = getIntR(0,validEntry-1); // просим ввести значения для изменения 
                tmpptr = ptr + inputKey;
            ...
            case 3:
            {
                ...
                {
                    tmpptr = ptr + i;  
                    cout << to_string(tmpptr->key) + " " + tmpptr->LastName + " " + tmpptr->FirstName + " " + tmpptr->LastName + 
                    " " + tmpptr->subject + " " + to_string(tmpptr->Ocenka) << endl;
                }
            }
        }
        idMenu = getIdMenu();
...
int getIntR(int form, int before) // диапозон целочисленной переменной
{
    int a = getInt();
